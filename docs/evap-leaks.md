...
| Calculator path | `TotalActivityGenerator` → `SourceBinDistributionGenerator` → `EvaporativeEmissionsOperatingModeDistributionGenerator` → **`LiquidLeakingCalculator`** → output aggregation |
