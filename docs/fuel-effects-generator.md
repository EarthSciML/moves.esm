...
The port specification for Phase 6's first rung, written to the method of
...
| Rows | 58 in `process-pm-exhaust`; **97 across the whole snapshot corpus** (§7.1), from two of its 42 snapshots |
...
**Six of the fourteen do not load the generator on their own, and that is
measured rather than argued.** Every snapshot carries an `execution-trace.json`
recording the Java classes MOVES actually loaded, so the question "did the
generator run here?" has a direct answer that does not depend on reading any
output table. Over the 40 snapshots that carry `generalFuelRatioExpression`:

| RunSpec processes | snapshots | `FuelEffectsGenerator` loaded |
|---|---:|---|
| contain one of 1, 2, 9, 10, 11, 12, 13, 90 | 22 | **yes**, all 22 |
| NONROAD (`<model value="NONROAD">`) | 12 | no, none |
| `{15}`, `{16}`, `{17}`, `{91}` alone | 6 | **no**, none |

so the predicate the oracle asserts is

> loaded ⇔ ONROAD **and** the run's processes meet {1, 2, 9, 10, 11, 12, 13, 90}

and it decides all 40 correctly. `process-crankcase-running` selects processes
1 **and** 15 and loads it; `process-crankcase-start` selects 16 alone and does
not; `process-apu` selects 91 alone and does not; `process-extended-idle`
selects 90 alone and does. Processes 18 and 19 never appear alone in the corpus
— `process-refueling` selects 1, 2, 18, 19 and 90 together — so their
individual behaviour is **untested here** and the eight-process set above is a
statement about what the corpus shows, not a claim to have read
`subscribeToMe`.
...
cases are the evidence, and eighteen of the forty are negative.
...
scan of the same tables, over all 42 snapshots as this is written; the corpus
grows and these are its counts on that day.

| | count |
|---|---:|
| `generalFuelRatioExpression` rows, summed over the corpus | **1,744** |
| distinct rows (all nine columns) | **558** |
| distinct `fuelEffectRatioExpression` strings | **116** |
| distinct parse-tree skeletons (every numeric literal replaced by `#`) | **70** |
| distinct top-level algebraic **forms** | **6** |

The gap between 70 and 6 is the whole reason the forms are worth factoring: the
...
one form and one coefficient set; only the text differs.
...
| B | `1 + (least(bioDieselEsterVolume, 20)/100)·c` | 650 | 4 | nineteen snapshots | **yes** — `biodiesel_ester_ratio` |
| C | `if(sulfurLevel > 30, HIGH, LOW)` | 923 | 100 | thirteen snapshots | **yes** — `sulfur_switch_ratio` |
| D | `((c₂·O² + c₁·O + c₀)/base)·scale` | 29 | 2 | the two air-toxics snapshots | no (§8.1) |
| E | `((benzeneContent − c)·k + 1)·scale` | 29 | 2 | the two air-toxics snapshots | no (§8.1) |
| F | `exp(A)/(exp(B) − exp(C))` | 55 | 4 | the two air-toxics snapshots | no (§8.1) |
| G | a bare numeric literal | 26 | 2 | `nr-airtoxics-lawn-garden-county` only | no — and it needs no template |

Row and string counts sum to 1,744 and 116 exactly, which is the check that the
classification is a partition and not a sample. Form G is a constant ratio at
polProcessID 2701 in one NONROAD snapshot, where the generator never runs at
all (§0.1), so it has no reachable answer to reproduce.
...
Forms A, B and C are **1,605 of the 1,744 rows**, and they are the three
...
`generalFuelRatio` carries two ratios. **On all 1,744 corpus rows the two
...
22 of them and deliberately did not run in 18. It **asserts** a floor under the
...
```
generalFuelRatioExpression: 40 snapshots carry it; MOVES loaded the generator in 22 and not in 18
               97 rows compared, 194 cells, 0 missing / 669 extra keys, 0 extras unaccounted for
               worst relative error 1.608e-14 at process-pm-exhaust fuel 5 formulation 27002 polProcess 11201 sourceType 32
subscription:  40 snapshots checked against ONROAD x processes {1,2,9,10,11,12,13,90}; 0 disagree
...
**1.608 × 10⁻¹⁴** relative, 0 missing keys, and 669 extra keys every one of
...
**The measurement: the corpus holds ZERO such expressions.** Parsing all 116
...
Measured: the port computes **766** rows over the 22 snapshots that ran the
generator, MOVES keeps **97** of them, and all **669** of the difference are
...
assertion, and it is what makes 669 extra keys evidence rather than a failure.

What follows for the port:

* **Form C is ported and no assertion anywhere faces MOVES on it.** 923 of the
  corpus's 1,744 expression rows, 100 of its 116 strings — the dominant form —
...
* **Forms D, E and F are not ported.** 113 rows and 8 strings between them, 48
  of those rows in `process-airtoxics` where all of them are handed to
  `ATRatio`, and the other 65 in `nr-airtoxics-lawn-garden-county`, a NONROAD
  snapshot where the generator never runs. They are
...
|---|---|---|
| the E85 `least(10, ETOHVolume)` clamp | 2 of 5 form-A strings | **no.** The only E85 formulation any corpus fuel supply carries is 27002, whose `ETOHVolume` is exactly 10.0, so the clamp is the identity on all 194 cells and deleting it leaves the comparison bit-identical. Probed instead: §6.4 |
...
polProcessIDs 1000101 and 1000102, 36 rows across five snapshots — is handed off
