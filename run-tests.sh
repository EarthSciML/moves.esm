...
if [[ ! -d "$SNAPSHOTS" ]]; then
  ...
  for oracle in "${ORACLES[@]}"; do
    ...
    if out=$(SNAPSHOTS="$SNAPSHOTS" $oracle 2>&1); then
      ...
      grep -E "rows compared|worst relative error|key set:|rows,|NOTE:|missing / |subscription:|fully stored|storage-limited|half-quanta|FLOAT columns:|int/int:|scheduling predicate|day axis:|collapse:|cells,|reproduced exactly|DECIMAL\(|alternative  misses|brackets |copy [0-9]|bit-exact|class-loaded as|snapshots with the tables|calculator-dag.json entry|TankVaporVenting" <<<"$out" \
        | sed 's/^/       /'
    else
