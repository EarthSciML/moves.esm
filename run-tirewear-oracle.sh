#!/usr/bin/env bash
#
# Run the independent reproduction of `process-tirewear` that lives in
# `docs/process-tirewear.md` §6.5.
#
...
# anyone for long.
#
#   ./run-tirewear-oracle.sh
#
# What it proves. It computes the WHOLE chain from the snapshot's own INPUT
# tables -- the activity half (specification §2.1), the cohort structure, BOTH
# operating-mode families (the drive-cycle weights for running exhaust and the
# speed-bin weights for tire wear, §2.3), the base rate for both computed
...
#
# It also asserts the three structural facts §3.2 rests on and no comparison
# against MOVESOutput could catch, because the mode families are disjoint in
# `emissionrate` and a union would still give the right answer: that each family
# sums to 1, that the drive-cycle family is 23 modes wide and the tire-wear one
# 16 -- not 17 -- and that operating mode 400 carries no weight.
#
# IT TAKES NOTHING FROM THE REFERENCE. `baserate_1_2020`, `baserate_10_2020`,
# `ratesopmodedistribution`, `sourcebindistribution` and `MOVESOutput` are read
# by nothing here except the final comparison.
#
